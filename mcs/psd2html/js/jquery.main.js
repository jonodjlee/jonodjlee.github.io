...
function initAnchors(){
		...
		jQuery('a.anchor').on('click', function(e) {
			e.preventDefault();
...
// initFixHeightBoxOnDevice();
function initFixHeightBoxOnDevice() {
	var holder = jQuery('.visual.win-height');
	if (holder && holder.length) {
		ResponsiveHelper.addRange({
			'..1024': {
				on: function() {
					holder.removeClass('win-height');
				},
				off: function() {
					holder.addClass('win-height');
				}
			},
		});
	}
...
function initLatestRelease() {
   jQuery.ajax({
	url:'https://api.github.com/repos/w3co/jcf/tags',
	jsonp: "callback",
	dataType: "jsonp",
	data: {
	format: "json"
	...
	if (response && response.data && response.data[0] && response.data[0].zipball_url){
		var url =  response.data[0].zipball_url;
			if (url.indexOf('w3co-jcf') >= -1){
			initLightbox(url);
			}
		}
	}
		...
		link.fancybox({
			...
			onComplete: function(box) {
				if (url !== undefined){
					...
					jQuery('.title-lightbox').text('Sorry, download is unavailable at the moment.');
				}	
				if(link.attr('href').indexOf('#') === 0) {
					jQuery('#fancybox-content').find('a.close').unbind('click.fb').bind('click.fb', function(e){
						jQuery.fancybox.close();
                                                jQuery('#popup1').removeClass('success');
                                                jQuery('#popup1').removeClass('error');
                                                jQuery('.mc-msg').html('Enter your email to get deals and news from PSD2HTML.');
						e.preventDefault();
					});
				}
			}
...
function initValidation() {
	...
	$('form.validate-form').each(function(){
		...
		function validateForm() {
			...
			var email = $('#uemail').serialize();
			if(successFlag) {
                                jQuery.ajax({
                                    url: form.attr('action') + encodeURIComponent(email),
                                    jsonp: "c",
                                    dataType: "jsonp",
                                    data: email,
                                    success: function( response ) {
                                        ...
                                        if (response.result == 'success') {
                                            form.parents('.lightbox').addClass(successClass);
                                            cleanField();
                                        } else if(response.result == 'error'){
                                            if(response.msg.indexOf('is already subscribed') + 1) {
                                                errorMsg = 'This email is already subscribed to PSD2HTML newsletter.';
                                            } else {
                                                errorMsg = 'Too many subscribe attempts. Please try again in 5 minutes.';
                                            }
                                            jQuery('.mc-msg').html(errorMsg);
                                            form.parents('.lightbox').addClass('error');
                                            cleanField();
                                        }
                                    }
                                });
		...
		function checkField(i, obj) {
			...
			var currentParent = currentObject.parents('.lightbox');
